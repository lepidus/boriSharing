...
function testExistNotificationToUser(){
    ...
    cy.get('.app__notifications > :nth-child(1) > :nth-child(2)').contains('Enabled API').should('exist');
    cy.get('.app__notifications > :nth-child(1) > :nth-child(3)').contains('Plugin working').should('exist');
}

function testDisabledAPI(){
    ...
    cy.get('button[id^=submitFormButton]').contains('Save').click();
    cy.get('.app__notifications > :nth-child(1) > :nth-child(4)').contains('Disabled API').should('exist');
}

describe('BoriSharing Plugin auth key test', function() {
    it('Adds API toggle tests: start enabled', function() {
